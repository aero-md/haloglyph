...
/**
 * Glyph Toy « Plumb » : un niveau à bulle, et une boussole.
 *
 ...
 *
 * Parce que son échelle n'est pas proportionnelle — voir `PlumbScale`. Un niveau
 ...
 * Voir `PlumbEngine`. Ce qui reste est la convention du pack, et rien de plus :
 ...
 */
class PlumbToyService : MatrixToyService(TAG) {

    private val engine = PlumbEngine()
    private val renderer by lazy { PlumbRenderer(spec, fromBack = true) }

    private lateinit var prefs: SharedPreferences
    private var sensors: PlumbSensors? = null
    private var mode = PlumbMode.DEFAULT
    ...
    override fun onGlyphConnected(context: Context, manager: GlyphMatrixManager) {
        prefs = PlumbConfig.prefs(context)
        mode = PlumbConfig.mode(prefs)
        engine.range = PlumbConfig.range(prefs)
        sensors = PlumbSensors(context, engine).also {
            it.compass = mode == PlumbMode.BOUSSOLE
            it.start()
        }
    ...
    override fun onTouchPointLongPress() {
        mode = mode.next
        PlumbConfig.setMode(prefs, mode)
        sensors?.compass = mode == PlumbMode.BOUSSOLE
    ...
    override fun renderFrame(frame: Frame, elapsedSeconds: Double, animated: Boolean) {
        ...
        engine.range = PlumbConfig.range(prefs)
        val next = PlumbConfig.mode(prefs)
        if (next != mode) {
            mode = next
            sensors?.compass = mode == PlumbMode.BOUSSOLE
        }
