...
import red.suns.haloglyph.plumb.toy.PlumbSensors
import java.util.concurrent.ConcurrentHashMap

/**
 * Plumb dans le hublot d'écran d'accueil — et c'est sans doute là qu'il est le
 * plus utile.
 *
 ...
 * différents — et la seconde est mise à l'endroit par [PlumbRenderer], puisqu'on
 ...
 */
class PlumbWidgetToy : WidgetToy {

    override val id: String = PlumbConfig.TOY_ID
    override val nameRes: Int = R.string.toy_plumb_name

    /** Le hublot se regarde de face : l'abscisse n'est pas celle de la matrice. */
    private val renderer by lazy { PlumbRenderer(fromBack = false) }
    ...
    private class Rig(val engine: PlumbEngine = PlumbEngine()) {
        var sensors: PlumbSensors? = null
    }

    private fun modeOf(context: Context): PlumbMode = PlumbConfig.mode(PlumbConfig.prefs(context))
     ...
     * avant de l'avoir réveillé. Voir [PlumbRenderer.renderIdle].
     */
    ...
    override fun onTap(context: Context, widgetId: Int, live: Boolean): Boolean {
        if (!live) return true
        val prefs = PlumbConfig.prefs(context)
        PlumbConfig.setMode(prefs, PlumbConfig.mode(prefs).next)
        return false
    }
    ...
    override fun onBurstStart(context: Context, widgetId: Int) {
        ...
        rig.engine.reset()
        rig.engine.range = PlumbConfig.range(PlumbConfig.prefs(context))
        rig.sensors = PlumbSensors(context, rig.engine).also {
            it.compass = modeOf(context) == PlumbMode.BOUSSOLE
            it.start()
        }
    }
    ...
    override fun renderBurst(
        ...
        val prefs = PlumbConfig.prefs(context)
        val mode = PlumbConfig.mode(prefs)
        ...
        rig.engine.range = PlumbConfig.range(prefs)
        rig.sensors?.compass = mode == PlumbMode.BOUSSOLE
