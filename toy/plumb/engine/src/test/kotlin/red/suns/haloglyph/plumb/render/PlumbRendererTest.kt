package red.suns.haloglyph.plumb.render
...
import red.suns.haloglyph.plumb.engine.PlumbRange
import red.suns.haloglyph.plumb.engine.PlumbScale
...
class PlumbRendererTest {
    ...
    private fun draw(fromBack: Boolean, r: PlumbEngine.Reading, mode: PlumbMode): Frame {
        val frame = Frame(spec)
        PlumbRenderer(spec, fromBack).render(frame, r, mode, PlumbRange.NORMALE, 0.0)
        return frame
    }

    private fun idle(mode: PlumbMode): Frame {
        val frame = Frame(spec)
        PlumbRenderer(spec, fromBack = false).renderIdle(frame, mode)
        return frame
    }
    ...
    fun `la bulle monte, vue de devant comme de derriere`() {
        ...
        assertTrue(bubble(draw(true, r, PlumbMode.NIVEAU)).second < spec.centerY)
        assertTrue(bubble(draw(false, r, PlumbMode.NIVEAU)).second < spec.centerY)
    }
    ...
    fun `le miroir inverse l'abscisse de la bulle`() {
        ...
        assertTrue(
            ...
            bubble(draw(true, r, PlumbMode.NIVEAU)).first < spec.centerX,
        )
        assertTrue(
            "sur un hublot, la bulle devrait partir à droite",
            bubble(draw(false, r, PlumbMode.NIVEAU)).first > spec.centerX,
        )
    }

    @Test
    fun `a plat la bulle est centree`() {
        val (x, y) = bubble(draw(true, reading(), PlumbMode.NIVEAU))
    ...
    fun `le niveau n'a aucun bord degrade`() {
        val frame = draw(true, reading(tilt = 3f, dirX = 0.6f, dirY = 0.8f), PlumbMode.NIVEAU)
    ...
    fun `la boussole n'a aucun bord degrade`() {
        val frame = draw(true, reading(heading = 37f), PlumbMode.BOUSSOLE)
        val levels = spec.leds.map { frame.values[it] }.filter { it > 0f }.distinct()
        assertTrue("trop de nuances : $levels", levels.size <= 3)
    }

    /** Le rayon de la couronne de visée, tel que le renderer le calcule. */
    private val target = spec.maxDistance * PlumbScale.TARGET_FRACTION
    ...
    fun `la visee ne change jamais`() {
        val loose = draw(true, reading(tilt = 3f, dirY = 1f), PlumbMode.NIVEAU)
        val locked = draw(true, reading(tilt = 0.2f, dirY = 1f, level = true), PlumbMode.NIVEAU)
        for (i in spec.leds) {
    ...
    fun `la visee n'entame pas la bulle`() {
        val frame = draw(true, reading(tilt = 0.6f, dirY = 1f), PlumbMode.NIVEAU)
    ...
    fun `cap au nord, le n est en haut du disque`() {
        for (fromBack in listOf(true, false)) {
            val frame = draw(fromBack, reading(heading = 0f), PlumbMode.BOUSSOLE)
            assertEquals(
    ...
    fun `les quatre lettres tiennent les quatre directions`() {
        val frame = draw(false, reading(heading = 0f), PlumbMode.BOUSSOLE)
    ...
    fun `la rose est la meme des deux cotes`() {
        for (heading in listOf(0f, 37f, 90f, 214f)) {
            ...
            val back = draw(true, r, PlumbMode.BOUSSOLE)
            val front = draw(false, r, PlumbMode.BOUSSOLE)
            for (i in spec.leds) {
    ...
    fun `cap a l'est, le nord passe a gauche`() {
        for (fromBack in listOf(true, false)) {
            val frame = draw(fromBack, reading(heading = 90f), PlumbMode.BOUSSOLE)
    ...
    fun `les lettres ne se deforment pas en tournant`() {
        for (heading in 0 until 360 step 3) {
            ...
            val frame = draw(true, reading(heading = h), PlumbMode.BOUSSOLE)
            for (letter in LETTERS) {
                assertEquals(
    ...
    fun `il n'y a plus d'intercardinaux`() {
        val frame = draw(true, reading(heading = 0f), PlumbMode.BOUSSOLE)
    ...
    fun `la rose n'a plus de cercle continu`() {
        val frame = draw(true, reading(heading = 20f), PlumbMode.BOUSSOLE)
    ...
    fun `l'aiguille a la pointe evidee`() {
        val frame = draw(true, reading(heading = 120f), PlumbMode.BOUSSOLE)
        val c = spec.centerX
    ...
    fun `sans cap, il n'y a pas de rose`() {
        val frame = draw(true, reading(hasHeading = false), PlumbMode.BOUSSOLE)
    ...
    fun `le repos porte la marque du toy et TAP`() {
        ...
        assertEquals("la fiole n'est pas entière", 45, band(idle(PlumbMode.NIVEAU), 13..21))
        assertEquals("la rose n'est pas entière", 29, band(idle(PlumbMode.BOUSSOLE), 13..21))
    }
