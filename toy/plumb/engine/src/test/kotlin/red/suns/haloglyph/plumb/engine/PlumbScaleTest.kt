...
class PlumbScaleTest {
    ...
    fun `zero au centre, pleine echelle au bord`() {
        for (range in PlumbRange.entries) {
            assertEquals(
                "${range.name} à plat",
                0f,
                PlumbScale.radiusOf(0f, range.degrees, maxRadius),
                1e-6f,
            )
            assertEquals(
                "${range.name} à pleine échelle",
                maxRadius,
                PlumbScale.radiusOf(range.degrees, range.degrees, maxRadius),
                1e-3f,
            )
        }
    }
    ...
    fun `l'echelle sature au lieu de deborder`() {
        val r = PlumbScale.radiusOf(180f, PlumbRange.NORMALE.degrees, maxRadius)
    ...
    fun `strictement croissante`() {
        ...
        var degrees = 0f
        while (degrees <= 15f) {
            val r = PlumbScale.radiusOf(degrees, PlumbRange.NORMALE.degrees, maxRadius)
    ...
    fun `un degre se voit, ce qu'une echelle lineaire ne fait pas`() {
        val full = PlumbRange.NORMALE.degrees
        val linear = maxRadius * 1f / full
        val log = PlumbScale.radiusOf(1f, full, maxRadius)
    ...
    fun `un dixieme de degre deplace la bulle d'au moins une cellule`() {
        for (range in PlumbRange.entries) {
            val r = PlumbScale.radiusOf(0.1f, range.degrees, maxRadius)
            assertTrue("${range.name} : 0,1° ne déplace que $r cellule(s)", r >= 0.9f)
        }
        assertTrue(PlumbScale.radiusOf(0.1f, PlumbRange.FINE.degrees, maxRadius) >= 1.4f)
    }
    ...
    fun `la portee ne coute presque rien pres du centre`() {
        val fine = PlumbScale.radiusOf(0.5f, PlumbRange.FINE.degrees, maxRadius)
        val wide = PlumbScale.radiusOf(0.5f, PlumbRange.LARGE.degrees, maxRadius)
    ...
    fun `tiltAt est l'inverse de radiusOf`() {
        for (range in PlumbRange.entries) {
            for (degrees in listOf(0.1f, 0.5f, 1f, 3f, range.degrees)) {
                val r = PlumbScale.radiusOf(degrees, range.degrees, maxRadius)
                assertEquals(
                    "${range.name} à $degrees°",
                    degrees,
                    PlumbScale.tiltAt(r, range.degrees, maxRadius),
                    1e-2f,
                )
            }
        }
    ...
    fun `la tolerance est celle du centre, quelle que soit la matrice`() {
        for (range in PlumbRange.entries) {
            val expected = PlumbScale.tolerance(range)
            for (radius in listOf(4f, 12.4f, 40f)) {
                assertEquals(
                    "${range.name} sur un rayon de $radius",
                    expected,
                    PlumbScale.tiltAt(PlumbScale.LOCK_FRACTION * radius, range.degrees, radius),
                    1e-4f,
                )
            }
        }
    ...
    fun `les tolerances annoncees`() {
        assertEquals(0.028f, PlumbScale.tolerance(PlumbRange.FINE), 0.004f)
        assertEquals(0.038f, PlumbScale.tolerance(PlumbRange.NORMALE), 0.004f)
        assertEquals(0.048f, PlumbScale.tolerance(PlumbRange.LARGE), 0.005f)
    }
    ...
    fun `au seuil, la bulle est encore au centre`() {
        for (range in PlumbRange.entries) {
            val tolerance = PlumbScale.tolerance(range)
            val inside = PlumbScale.radiusOf(tolerance, range.degrees, maxRadius)
            assertTrue("${range.name} : $inside cellule(s)", Math.round(inside) == 0)
            val outside = PlumbScale.radiusOf(tolerance * 2.5f, range.degrees, maxRadius)
