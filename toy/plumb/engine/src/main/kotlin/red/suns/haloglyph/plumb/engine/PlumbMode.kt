 ...
 */
enum class PlumbMode {

    /** La bulle. Voir [PlumbScale] pour ce qui la sépare d'un niveau ordinaire. */
    NIVEAU,

    /** La rose des vents, et le cap en chiffres au centre. */
    BOUSSOLE;

    val next: PlumbMode get() = entries[(ordinal + 1) % entries.size]

    companion object {
        val DEFAULT = NIVEAU

        /** Lecture tolérante d'une préférence : une clé inconnue retombe au défaut. */
        fun byKey(key: String?): PlumbMode = entries.firstOrNull { it.name == key } ?: DEFAULT
    }
}
