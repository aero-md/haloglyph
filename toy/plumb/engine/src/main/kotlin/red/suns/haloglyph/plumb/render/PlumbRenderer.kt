...
class PlumbRenderer(
    ...
    /** La couronne de visée. Son rayon est la tolérance, voir [PlumbScale]. */
    private val targetRadius: Float = usableRadius * PlumbScale.TARGET_FRACTION
    ...
    fun render(
        frame: Frame,
        reading: PlumbEngine.Reading,
        mode: PlumbMode,
        range: PlumbRange,
        elapsedSeconds: Double,
    ) {
        when (mode) {
            PlumbMode.NIVEAU -> renderLevel(frame, reading, range)
            PlumbMode.BOUSSOLE -> renderCompass(frame, reading, elapsedSeconds)
        }
    }
     ...
     * hublot posé sur Plumb est tantôt un niveau, tantôt une boussole, le tap
     ...
     */
    fun renderIdle(frame: Frame, mode: PlumbMode) {
        val badge = if (mode == PlumbMode.BOUSSOLE) ROSE else VIAL
    ...
    private fun renderLevel(frame: Frame, reading: PlumbEngine.Reading, range: PlumbRange) {
        ...
        val r = PlumbScale.radiusOf(reading.tiltDegrees, range.degrees, usableRadius)
     ...
     *
     * La bulle n'y a qu'une dimension de liberté — voir `PlumbEngine`, qui ne
     ...
     */
    private fun renderCompass(frame: Frame, reading: PlumbEngine.Reading, elapsedSeconds: Double) {
