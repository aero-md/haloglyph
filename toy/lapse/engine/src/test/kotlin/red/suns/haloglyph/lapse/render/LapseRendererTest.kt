...
class LapseRendererTest {
    ...
    /* Langue épinglée, et pas le défaut de la JVM : `LapseRenderer()` prend
       les étiquettes de `Locale.getDefault()`, si bien qu'un runner en anglais
       ferait échouer les assertions sur « J » plus bas — sans rien dire d'utile
       sur le rendu. Ce qui vaut pour les cinq langues est dans
       `MatrixLabelsTest` ; ici on vérifie la géométrie, à langue constante. */
    private val renderer = LapseRenderer(labels = MatrixLabels.FR)
    ...
    fun `anneau - nombre de cellules proportionnel aux secondes`() {
        ...
        renderer.renderRing(frame, fullDiff) // 45 s
        val expected = (fullDiff.seconds / 60.0 * renderer.ring.size).roundToInt()
        assertEquals(expected, frame.values.count { it > 0f })
    }
    ...
    fun `sablier - quantite de sable fidele et croissante`() {
        ...
        val target30 = (30 / 60.0 * MatrixSpec.Phone3.ledCount).roundToInt()
    ...
    fun `texte - jamais clippe pour les formats détail sur diff complete`() {
        ...
        fun assertFits(f: Font, s: String, y: Int) {
            ...
            assertEquals("clipping pour '$s' à y=$y", expected, frame.values.count { it > 0f })
        }
    ...
    fun `arrivee - flash initial plein disque`() {
        ...
        val litRatio = frame.count { it > 100 } / MatrixSpec.Phone3.ledCount.toDouble()
